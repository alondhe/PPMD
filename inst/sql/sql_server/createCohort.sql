...
SELECT C.person_id, C.episode as event_id, C.episode_end_date as start_date, episode_end_date as end_date
FROM 
(
  SELECT A.*, row_number() over (PARTITION BY A.person_id ORDER BY A.episode_start_date, A.episode) as ordinal
  from @resultsDatabaseSchema.pregnancy_episodes A
  where A.outcome <> 'SA/AB'
  @whereClause
) C

JOIN @cdm_database_schema.PERSON P on C.person_id = P.person_id
WHERE (C.episode_end_date >= DATEFROMPARTS(2009, 01, 01) and C.episode_end_date <= DATEFROMPARTS(2012, 12, 31))
AND (YEAR(C.episode_end_date) - P.year_of_birth >= 18 and YEAR(C.episode_end_date) - P.year_of_birth <= 40)
