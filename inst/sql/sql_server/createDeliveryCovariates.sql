...
as
(
  select distinct A.subject_id as row_id, 1 as covariate_id, 1 as covariate_value, 
...
as
(
  select distinct A.subject_id as row_id, 2 as covariate_id, 1 as covariate_value,
  row_number() over (PARTITION BY B.person_id ORDER BY B.episode_end_date, B.episode) as ordinal
  from @cohortTable A
  join @resultsDatabaseSchema.pregnancy_episodes B on A.subject_id = B.person_id
    and A.cohort_start_date = B.episode_end_date
...
as
(
  select distinct A.subject_id as row_id, 3 as covariate_id, 1 as covariate_value,
  row_number() over (PARTITION BY B.person_id ORDER BY B.episode_end_date, B.episode) as ordinal
  from @cohortTable A
  join @resultsDatabaseSchema.pregnancy_episodes B on A.subject_id = B.person_id
    and A.cohort_start_date = B.episode_end_date
